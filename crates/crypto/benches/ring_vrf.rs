...
fn gen_items(n: usize) -> (Verifier, Vec<(Vec<u8>, [u8; 784])>) {
    assert!(
        n <= RING_SIZE,
        "batch size {n} exceeds RING_SIZE {RING_SIZE}"
    );
    let ring: Vec<KeyPair> = (0..RING_SIZE)
        .map(|i| KeyPair::from([i as u8; 32]))
        .collect();
    let pks = ring.iter().map(|k| k.public().unwrap()).collect::<Vec<_>>();
...
fn bench(c: &mut Criterion) {
    ...
    for &n in [1usize, 3, 8, 16, 32, 64]
        .iter()
        .filter(|&&n| n <= RING_SIZE)
    {
            ...
            b.iter(|| {
                verifier
                    .ring_vrf_verify_batch(
                        items
                            .iter()
                            .map(|(m, s)| (m.as_slice(), [].as_slice(), s.as_slice())),
                    )
                    .unwrap()
            })
        });
