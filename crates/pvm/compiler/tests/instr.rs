...
mod debug_tests {
    use super::*;
    use parser::ProgramBlob;

    #[test]
    fn debug_jump_indirect_execution_flow() {
        println!("=== ANALYZING JUMP_INDIRECT EXECUTION FLOW ===");
        
        // Get the specific test by loading it like the generated tests
        let test = specjam::Registry::new("../../../res/jam-test-vectors")
            .entry("pvm").unwrap()
            .into_iter()
            .find(|t| t.name == "jump_indirect_without_offset_ok")
            .expect("Test not found");
        
        println!("Test: {}", test.name);
        
        // Parse the input to get the program
        let input: TestInput = serde_json::from_str(&test.input).unwrap();
        println!("Program length: {}", input.program.len());
        
        // Parse the program to see the instructions
        let program_blob = ProgramBlob::try_from(input.program.as_slice()).unwrap();
        println!("Jump table: {:?}", program_blob.jump_table);
        
        let mut reader = program_blob.reader();
        let mut pc = 0;
        
        println!("=== INSTRUCTIONS ===");
        while !reader.eof() {
            if let Ok(offset_instruction) = reader.read() {
                println!("PC={:2} ({}..{}): {:?}", pc, offset_instruction.range.start, offset_instruction.range.end, offset_instruction.value);
                pc = offset_instruction.range.end;
            } else {
                break;
            }
        }
        ...
        let output: TestOutput = serde_json::from_str(&test.output).unwrap();
        println!("\n=== EXPECTED STATE ===");
        println!("Expected registers: {:?}", output.expected_regs);
        println!("Expected PC: {}", output.expected_pc);
        
        println!("\n=== RUNNING COMPILER TEST ===");
        let result = Runner::step(&test);
        match result {
            Ok(_) => println!("✅ PASS: Compiler test succeeded"),
            Err(e) => println!("❌ FAIL: Compiler test failed: {}", e),
        }
    }
}
